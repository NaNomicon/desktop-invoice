...
function nextBlankLineItems(companyIds: number[] = []): LineItem[] {
  if (companyIds.length === 0) {
    return [{ uid: nextUid(), id: 0, qty: 1, product_id: null, product_name: '', unit_price: 0, row_total: 0, s_no: 1, deleted: false, company_id: null }];
  }
  return companyIds.map((cid, i) => ({
    uid: nextUid(), id: 0, qty: 1, product_id: null, product_name: '',
    unit_price: 0, row_total: 0, s_no: i + 1, deleted: false, company_id: cid,
  }));
}
...
function ProductRow({ li, idx, companyProducts, currency, onProductSelect, onQtyChange, onDelete }: ProductRowProps) {
  ...
  return (
    <tr className={`border-t hover:bg-muted/30 ${li.deleted ? 'bg-muted/20 opacity-50' : ''}`}>
      ...
      <td className="px-3 py-1.5">
        <span className={li.deleted ? 'text-muted-foreground' : ''}>
...
function InvoiceForm() {
    ...
    if (companyIds.length > 0) {
      setLineItems((prev) => {
        ...
        if (prev.length === 1 && first && !first.product_id && first.company_id === null) {
          return nextBlankLineItems(companyIds);
        }
        return prev;
      });
    }
    ...
    (uid: string) => {
      setLineItems((prev) => {
        ...
        if (activeCount === 0) {
          ...
          return nextBlankLineItems(companies.map((c) => c.id));
        }
        return next;
      });
    },
  ...
  const resetForm = useCallback(
    (nextInvoiceNumber?: string) => {
      ...
      setDiscountMode('per');
      setPrintDue(false);
      setLineItems(nextBlankLineItems(companies.map((c) => c.id)));
