...
Public Class Demo_Add_Edit
    ...
    Public Sub load_data()
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            End If
            If ide = "Advance" Then
                copy_dua_amount.Text = "-" & Val(amount_due.Text)
            ElseIf ide = "Due" Then
                copy_dua_amount.Text = Val(amount_due.Text)
            End If
        End If
    ...
    Public Sub saved()
        ...
        ElseIf case_debit.Text = "CREDIT" Then
            ...
            If invoice_id > 0 Then
                Dim m1 As String = ""
                Dim get_due As Double = get_single_value("due_amount", "tbl_customer", "id", customer_id)
                m1 = "update tbl_customer set due_amount='" & Math.Abs(Val(get_due) - Val(load_amount)) & "' where id='" & customer_id & "'"
                cmd = New SqlCommand(m1, con)
                cmd.ExecuteNonQuery()
        ...
        Else
            If invoice_id > 0 Then
                ...
                Dim d As String = SQL_Update("tbl_invoice_main", variable, "id=" & invoice_id)
            Else
